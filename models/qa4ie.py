...
class QA4IEQA(nn.Module):
    def __init__(self, config):
        ...
        self.encode_lstm = nn.LSTM(d, config.hidden_size, batch_first=True, bidirectional=True)
        self.biattention = BiAttentionFlow(config.hidden_size)
        self.model_lstm = nn.LSTM(8 * config.hidden_size, config.hidden_size, num_layers=config.num_modeling_layers,
    ...
    def forward(self, char_context, char_query, context, query, context_mask, query_mask, labels=None, labels_mask=None):
        ...
        ctx, _ = self.encode_lstm(ctx)  # [N, Lc, 2d]
        q, _ = self.encode_lstm(q)  # [N, Lq, 2d]
        # attention flow
        q_aware_ctx = self.dropout(self.biattention(ctx, q, context_mask, query_mask))
        # modeling layer
        g, _ = self.model_lstm(q_aware_ctx)
        # self matching layer
        g, encoder_final_state = self.matching(g, context_mask)
        # pre pointer network
        output = self.ptrnet(g, encoder_final_state, context_mask)
        ...
        loss = (self.loss_fnt(output.reshape(N * La, -1), labels.reshape(-1)) * labels_mask.float().reshape(-1)).sum() / labels_mask.sum()
        return loss, output
...
#         self.highway = HighwayNetwork(config.num_highway_layers, d)
#         self.encode_lstm = nn.LSTM(d, config.hidden_size, batch_first=True, bidirectional=True)
#         self.bilinear = nn.Bilinear(2*config.hidden_size, 2*config.hidden_size, 64)
#         self.classifier = nn.Sequential(
#             nn.Dropout(config.dropout),
#             # nn.Linear(2 * config.hidden_size, 64),
#             nn.ReLU(),
...
#         ctx, q = self.emb(char_context, char_query, context, query)
#         ctx, q = self.highway(ctx), self.dropout(self.highway(q))
#         ctx, _ = self.encode_lstm(ctx)

#         q, _ = self.encode_lstm(q)
#         # print(ctx.size(), q.size(), context_mask.size(), query_mask.size())
#         ctx = (ctx * context_mask.unsqueeze(-1)).sum(dim=1) / context_mask.sum(dim=1)[:, None]
#         q = (q * query_mask.unsqueeze(-1)).sum(dim=1) / query_mask.sum(dim=1)[:, None]
#         f = self.bilinear(ctx, q)
